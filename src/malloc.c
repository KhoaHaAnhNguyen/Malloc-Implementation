...
struct _block 
{
   ...
   struct _block *prev;  /* keeping track of previous block*/
};
...
// \TODO Put your Best Fit code in this #ifdef block
   ...
   while(curr && !(curr->free && curr->size >= size && (best_fit == NULL || curr->size < best_fit->size ))) 
   {
      *last = curr;
      curr = curr->next;
   }
...
// \TODO Put your Worst Fit code in this #ifdef block
   ...
   while (curr && !(curr->free && curr->size >= size && (worst_fit == NULL || curr->size > worst_fit->size))) 
   {
      *last = curr;
      curr = curr->next;
   }
...
// \TODO Put your Next Fit code in this #ifdef block
   ...
   while (curr && !(curr->free && curr->size >= size && (next_fit == NULL || curr->size > next_fit->size))) 
   {
      *last = curr;
      curr = curr->next;
   }
...
void *malloc(size_t size) 
{
   ...
   */
   if (next != NULL) 
   {
      ...
      {
         // Split block 
         struct _block *new_block = (struct _block *) (BLOCK_DATA(next) + size);
   ...
   if (next == NULL) 
   {
      ...
      max_heap = num_blocks * sizeof(struct _block);
   }
...
void *realloc( void *ptr, size_t size )
{
   ...
   if (ptr == NULL) {
      /* ptr is NULL, so just call malloc */
      return malloc(size);
   }

   if (size == 0) {
      /* size is 0, so just call free */
      free(ptr);
      return NULL;
   }
   ...
   //Try to find a free block to reuse 
   struct _block *last = heapList;
   struct _block *new_block = findFreeBlock(&last, size);
   if (new_block) {
      // Found a free block to reuse 
      num_reuses++;

      //Check if the block can be split into two blocks //
      if (new_block->size - size >= sizeof(struct _block)) {
         /* Split the block into two */
         struct _block *split_block = (struct _block *)((char *)new_block + size);
         split_block->size = new_block->size - size - sizeof(struct _block);
